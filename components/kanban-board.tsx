    ...
    <Card className='w-100 shrink-0 shadow-md p-0'>
      ...
      >
        <div className='flex items-center justify-between'>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className='h-6 w-6 text-white hover:bg-white/20 rounded-md p-1 flex items-center justify-center'>
      ...
      <CardContent
        ref={setNodeRef}
        className={`space-y-2 pt-4 bg-gray-50/50 min-h-[100] rounded-bl-lg ${
...
export default function KanbanBoard({ board, userId }: KanbanBoardProps) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    if (targetColumn) {
      ...
      const targetJobsCOlumn = sortedColumns.find((col) =>
        col.jobApplications.some((job) => job._id === targetJob._id)
      )
      targetColumnId = targetJob.columnId || targetJobsCOlumn?._id || ""
